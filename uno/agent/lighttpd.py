  ...
  def start(self) -> None:
    ...
    try:
      ...
      self._lighttpd = subprocess.Popen(
        ["lighttpd", "-D", "-f", self._lighttpd_conf], preexec_fn=os.setpgrp
      )
      ...
      timer.wait()
      log.info(
        "lighttpd started ({}), listening on {} interfaces: {}",
        detected_pid.pop(),
      ...
      if lighttpd_started:
        ...
        log.error(
          "lighttpd process was started but possibly not stopped. Please check your system."
        )
      raise e
  ...
  def stop(self) -> None:
    ...
    try:
      if self._lighttpd_pid.is_file():
        ...
        exec_command(
          ["kill", "-s", "SIGTERM", str(pid)], fail_msg="failed to signal lighttpd process"
        )
