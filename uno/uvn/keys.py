...
class KeyId:
  ...
  @staticmethod
  def from_uvn_id(id: UvnId|CellId|ParticleId) -> "KeyId":
    if isinstance(id, UvnId):
      return KeyId(key_type=KeyId.Type.ROOT, owner=id.owner, target=id.name)
    elif isinstance(id, CellId):
      return KeyId(key_type=KeyId.Type.CELL, owner=id.owner, target=id.name)
    elif isinstance(id, ParticleId):
      return KeyId(key_type=KeyId.Type.PARTICLE, owner=id.owner, target=id.name)
    else:
      raise ValueError(id)
...
class KeysBackend(Versioned):
  ...
  def __init__(self, root: Path, **load_args) -> None:
    self.root = root.resolve()
    super().__init__(**load_args)


  def __eq__(self, other):
    if not isinstance(other, KeysBackend):
      return False
  ...
  def __getitem__(self, id: KeyId|UvnId|CellId|ParticleId) -> Key:
    key = self.get_key(id)
    if key is None:
      raise KeyError(id)
    return key
  ...
  def get_key(self, id: KeyId|UvnId|CellId|ParticleId) -> Key|None:
    if isinstance(id, (UvnId, CellId, ParticleId)):
  ...
  def random_passphrase(self) -> str:
    return secrets.token_urlsafe(KeysBackend.PASSPHRASE_LEN)
