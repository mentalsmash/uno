...
import random
from typing import Tuple, Callable, Sequence, Optional, Iterable, Iterator
...
class P2PLinksMap:
  ...
  @staticmethod
  def deserialize(serialized: dict) -> "P2PLinksMap":
    return P2PLinksMap(
      ...
      peers={
        ...
        } for peer_a_id, peer_a in serialized.get("peers", {}).items()
      })
            ...
            for i in range(deployed_peers_count(n))
              for peer_b_n in [deployed_peers_connections[i](n)] if peer_b_n is not None
                for peer_b in [deployed_peers[peer_b_n]]
                  for link_addresses, link_network in [network_map.assert_pair(peer_a, peer_b)]
        },
...
class StaticDeploymentStrategy(DeploymentStrategy):
  def __init__(self,
      ...
      args: Optional[dict]=None) -> None:
    super().__init__(peers, private_peers, args)
    self.static_deployment = tuple((p, tuple(peers))
...
#     self.private_peers = set(private_peers) if private_peers else []
#     super().__init__([])


#   def deploy(self, peer_count: int) -> Tuple[Sequence[int] | Callable[[int], int] | Sequence[Callable[[int], int]]]:
#     if peer_count <= 1:
#       return self.EMPTY_DEPLOYMENT
...
#     peer_ids = set(deployment_map.keys())
#     # random.shuffle(peer_ids)


#     allocated_min_peers = set()
#     available_ports = set(deployment_map) - self.private_peers
#     exhausted_ports = set()
#     remaining_private = peer_ids & self.private_peers
#     remaining_public = peer_ids - remaining_private

#     def _assign_port(peer_id: int, other_peer_id: int, peer_allocated: Sequence[int], other_peer_allocated: Sequence[int]):
#       # Allocate peer port
#       peer_allocated.append(other_peer_id)
#       other_peer_allocated.append(peer_id)

#       if len(peer_allocated) >= self.min_peers:
#         allocated_min_peers.add(peer_id)
#       if len(peer_allocated) >= self.max_peers:
#         exhausted_ports.add(peer_id)

#       if len(other_peer_allocated) >= self.min_peers:
#         allocated_min_peers.add(other_peer_id)
#       if len(other_peer_allocated) >= self.max_peers:
#         exhausted_ports.add(other_peer_id)


#     def _assign_from_set(available_peers) -> Optional[int]:
#       # available_peers = list(available_peers)
#       # random.shuffle(available_peers)
#       available_peers = sorted(available_peers, key=lambda v: len(deployment_map[v]), reverse=True)
#       other_peer_id = next(iter(available_peers), None)
#       if other_peer_id is None:
#         return None
#       other_peer_allocated = deployment_map[other_peer_id]
#       _assign_port(peer_id, other_peer_id, peer_allocated, other_peer_allocated)
#       return other_peer_id


#     for peers_set in [remaining_private, remaining_public]:
#       # Don't explicitly allocate more ports than the minimal.
#       for port_i in range(self.min_peers):
#         # peer_ids = list(set(deployment_map.keys()) - allocated_min_peers)
#         # random.shuffle(peer_ids)

#         peer_ids = list(peers_set - allocated_min_peers)
#         # random.shuffle(peer_ids)

#         for peer_id in peer_ids:
#           peer_allocated = deployment_map[peer_id]
#           # Check if the port was already assigned through another peer
#           if len(peer_allocated) >= port_i + 1:
#             continue

#           peer_public = peer_id not in self.private_peers

#           # Find the next suitable peer
#           available_min_peers = set()
#           available_public = set()
#           assigned = False
#           for other_peer_id in sorted(available_ports, key=lambda v: len(deployment_map[v]), reverse=True):
#             if other_peer_id == peer_id or other_peer_id in peer_allocated:
#               continue

#             other_peer_allocated = deployment_map[other_peer_id]
#             if peer_id in other_peer_allocated:
#               continue

#             other_public = other_peer_id not in self.private_peers
#             if peer_public and other_public:
#               available_public.add(other_public)

#             if other_peer_id in allocated_min_peers:
#               available_min_peers.add(other_peer_id)
#               continue
...
#           if not assigned and (available_public or available_min_peers):
#             if not peer_public:
#               preferred = available_min_peers - available_public
#               backup = available_public
#             else:
#               backup = available_min_peers - available_public
#               preferred = available_public
#             other_peer = _assign_from_set(preferred)
#             if not other_peer:
#               other_peer = _assign_from_set(backup)
#             assigned = other_peer is not None

#           if not assigned:
#             # Failed to allocate a peer, but if we have at least one
#             # we can just skip to the next peer
#             if port_i == 0 and self.min_peers > 0:
#               raise RuntimeError(f"failed to allocate at least one peer", peer_ids, peer_id, deployment_map)
#             log.warning(f"[ASYMM-STRATEGY] failed to allocate port {port_i} for peer {peer_id}")
#             continue

#           available_ports -= exhausted_ports


#     self.static_deployment = tuple(deployment_map.items())
#     return super().deploy(peer_count)
...
class RandomDeploymentStrategy(StaticDeploymentStrategy):
  ...
  def __init__(self,
    ...
    if peers_count <= 1:
      ...
      max_peers = 3
      if private_peers or public_count < private_count:
        max_peers = 3 + private_count - public_count

    if max(max_peers, min_peers, ok_peers) != max_peers:
      raise RuntimeError("invalid max_peers setting")
