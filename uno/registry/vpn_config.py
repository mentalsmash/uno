...
class CentralizedVpnConfig(Versioned):
  ...
  def prepare_peer_configs(
    self, val: str | dict | dict[int, WireGuardConfig]
  ) -> dict[int, WireGuardConfig]:
    def _mktuple(cls, v):
      peer_id, val = v
  ...
  def root_config(self) -> WireGuardConfig:
    return WireGuardConfig(
      intf=WireGuardInterfaceConfig(
        ...
        endpoint=f"{self.root_endpoint}:{self.settings.port}"
        if self.root_endpoint is not None
        else None,
    ...
    return WireGuardConfig(
      ...
      peers=[
        WireGuardInterfacePeerConfig(
          ...
          endpoint=None
          if self.root_endpoint is None or peer_endpoint
          else f"{self.root_endpoint}:{self.settings.port}",
  ...
  def peer_config(self, peer: int) -> list[WireGuardConfig]:
    ...
    return [
      ...
      for peer_b_id, (
        ...
        peer_b_address,
        link_network,
      ) in peer_deploy_cfg["peers"].items()
...
class UvnVpnConfig(Versioned):
  EQ_PROPERTIES = ["object_id"]

  @classmethod
  def root_vpn_config(
    cls, registry: "Registry", rekeyed: bool = False
  ) -> CentralizedVpnConfig | None:
    if not registry.uvn.settings.enable_root_vpn:
      return None

    cells = sorted(registry.uvn.all_cells.values(), key=lambda v: v.id)

    return registry.new_child(
      CentralizedVpnConfig,
      {
