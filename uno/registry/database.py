...
class Database:
  ...
  def next_id(self, target: DatabaseObject | type[DatabaseObject]) -> int:
    ...
    next_id = (
      self._db.execute("SELECT next FROM next_id WHERE target = ?", (table,)).fetchone().next
    )
    next_id += 1
    self._db.execute("UPDATE next_id SET next = ? WHERE target = ?", (next_id, table))
    return next_id

  def initialize(self) -> None:
    for script in ["initialize_registry.sql", "initialize_agent.sql"]:
    ...
    def do_save():
      ...
      for tgt, current_owner in query_targets.items():
        ...
        if tgt.disposed:
          ...
          tgt.save(cursor=cursor, create=create, import_record=import_record, public=public)
          tgt.clear_changed()
          tgt.saved = True
  ...
  def _delete(
    ...
    self.log.trace(
      "delete {} query: {} values {}", "rows" if cls is None else cls.__qualname__, query, params
    )
  ...
  def load(
    ...
    def _load_targets() -> Generator[tuple, None, None]:
      order_by_clause = (
        ""
        if not cls.DB_ORDER_BY
        else (
          " ORDER BY "
          + ", ".join(f"{col} {'ASC' if asc else 'DESC'}" for col, asc in cls.DB_ORDER_BY.items())
        )
      )
      ...
      if id is not None:
        ...
        if cached:
          yield cached
          return
        query = (f"SELECT * FROM {table} WHERE id = ?{order_by_clause}", (id,))
      ...
      # Load objects by making a custom query on their db table
        ...
        query = (f"SELECT * FROM {table} WHERE {where}{order_by_clause}", params)
      ...
      # Return only those objects matching the clause.
        ...
        owner_table, owner_col, owned_col = self.SCHEMA.lookup_owner_table_by_object(
          cls, owner_cls=owner.__class__
        )
        ...
        if owner_table == table:
          ...
          join_order_by_clause = (
            ""
            if not cls.DB_ORDER_BY
            else (
              " ORDER BY "
              + ", ".join(
                f"{table}.{col} {'ASC' if asc else 'DESC'}" for col, asc in cls.DB_ORDER_BY.items()
              )
            )
          )
          query = (
  ...
  def delete(
    ...
    def _delete():
      ...
      current_owners = {tgt: tgt.owner for tgt in targets if isinstance(tgt, OwnableDatabaseObject)}
      ...
      for tgt in _query_targets():
        ...
        self._delete(
          table, f"DELETE FROM {table} WHERE id = ?", [tgt.id], tgt.__class__, cursor=cursor
        )
  ...
  def deserialize(self, cls: type, serialized: dict, use_cache: bool = True) -> tuple[object, bool]:
    ...
    if loaded is None:
      if issubclass(cls, DatabaseObject):
        ...
        logger = (
          self.log.activity
          if loaded.DB_TABLE and loaded.DB_CACHED
          else self.log.debug
          if loaded.DB_TABLE
          else self.log.trace
        )
  ...
  def owner(
    ...
    for owner_cls in owner_types:
      owner_table, owner_col, owned_col = self.SCHEMA.lookup_owner_table_by_object(
        target, owner_cls=owner_cls
      )
      if owner_table in searched_tables:
        continue
      searched_tables.add(owner_table)
      result = cursor.execute(
        f"SELECT {owner_col} FROM {owner_table} WHERE {owned_col} = ?", (target_id,)
      ).fetchone()
      if result:
  ...
  def set_ownership(
    ...
    cursor: "Database.Cursor | None" = None,
    do_in_transaction: TransactionHandler | None = None,
  ) -> dict[DatabaseObjectOwner, bool]:
    if getattr(owner, "excluded", False) and owned:
      raise ValueError("cannot transfer ownership to excluded owner")

    def _set_owenership():
      current_owners = {tgt: self.owner(tgt, cursor=cursor) for tgt in targets}
      return self._set_ownership(
        owner, targets, owned=owned, current_owners=current_owners, cursor=cursor
      )
  ...
  def _set_ownership(
    ...
    for target in targets:
      ...
      if (owned and current_owner == owner) or (not owned and current_owner != owner):
        # Ownership unchanged
        self.log.debug("ownership unchanged for {}: {}", target, current_owner)
        continue
      owner_table, owner_col, owned_col = self.SCHEMA.lookup_owner_table_by_object(
        target, owner_cls=owner.__class__
      )
    ...
    def _export():
      ...
      for exported_cls in classes:
        ...
        owner_tables = (
          ...
          if issubclass(exported_cls, OwnableDatabaseObject)
          else []
        )
  ...
  def export_objects(
    self, target: "Database", objects: Iterable[Versioned], public: bool = False
  ) -> None:
