...
if False:
    def list_local_nics_netifaces(interfaces=[], skip=[], include_loopback=False) -> Sequence[tuple[str, Sequence[Mapping[str, str]]]]:
        """
...
def list_local_nics(interfaces: list[str]|None=None, skip: list[str]|None=None, include_loopback: bool=False) -> list[tuple[str, list[dict[str, ipaddress.IPv4Address|int|ipaddress.IPv4Network]]]]:
  result = {}
...
def ipv4_nic_network(nic_addr: str|ipaddress.IPv4Address, nic_netmask: int|str|ipaddress.IPv4Address=None) -> ipaddress.IPv4Network:
