...
_LogLevels = namedtuple(
  "LogLevels", ["tracedbg", "trace", "debug", "activity", "info", "warning", "error", "quiet"]
)
  ...
  with _LOGGER_LOCK:
    ...
    if syslog is None:
      ...
      log_fn = (
        ...
        if lvl >= logger.Level.warning
        else syslog.error
        if lvl >= logger.Level.error
        else syslog.critical
      )
      log_fn(line)
      if exc_info:
...
class UvnLogger:
  ...
  def parse_level(cls, val: _LogLevels | int | str | None) -> _LogLevels:
    if val is None or isinstance(val, int):
      val = (
        cls.Level.quiet
        if val is None
        else cls.Level.tracedbg
        if val >= 5
        else cls.Level.trace
        if val >= 4
        else cls.Level.debug
        if val >= 3
        else cls.Level.activity
        if val >= 2
        else cls.Level.info
        if val >= 1
        else cls.Level.warning
      )
  ...
  def syslog(self) -> logging.Logger | None:
    ...
    logger.setLevel(
      logging.DEBUG
      if self.level >= self.Level.trace
      else logging.INFO
      if self.level >= self.Level.info
      else logging.WARNING
      if self.level >= self.Level.warning
      else logging.ERROR
  ...
  def verbose_flag(self) -> str | None:
    verbosity_level = (
      5
      if self.level >= self.Level.tracedbg
      else 4
      if self.level >= self.Level.trace
      else 3
      if self.level >= self.Level.debug
      else 2
      if self.level >= self.Level.activity
      else 1
      if self.level >= self.Level.info
      else 0
    )
  ...
  def command(self, cmd_args, rc, stdout=None, stderr=None, display=False):
    if rc != 0:
      ...
      self.error("  stdout: {}", stdout.decode("utf-8") if stdout else "<none>")
      self.error("  stderr: {}", stderr.decode("utf-8") if stderr else "<none>")
    else:
      self.trace("  stdout: {}", stdout.decode("utf-8") if stdout else "<none>")
      self.trace("  stderr: {}", stderr.decode("utf-8") if stderr else "<none>")
