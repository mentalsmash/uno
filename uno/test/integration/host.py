...
class Host:
  ...
  def interfaces(self) -> dict[str, ipaddress.IPv4Address]:
    result = exec_command(
      ["ip -o a s | grep -E ': eth[0-9]+    inet ' | awk '{print $2 \" \" $4;}'"],
      shell=True,
      capture_output=True,
    ).stdout
    if not result:
  ...
  def test_file(self, name: str, create: bool = False) -> tuple[Path, Path]:
    ...
    return [
      output,
      self.experiment.RunnerTestDir
      / self.test_dir.relative_to(self.experiment.test_dir)
      / output.name,
    ]
  ...
  def cell_addresses(self) -> list[ipaddress.IPv4Address]:
    ...
    addresses.extend(self.experiment.registry.deployment.get_interfaces(self.cell.id))
    return addresses

  @property
  def cell_reachable_networks(self) -> set[ipaddress.IPv4Network]:
    return _read_networks_file(
      self.log, self.experiment_uvn_dir / "log" / Agent.REACHABLE_NETWORKS_TABLE_FILENAME
    )

  @property
  def cell_unreachable_networks(self) -> set[ipaddress.IPv4Network]:
    return _read_networks_file(
      self.log, self.experiment_uvn_dir / "log" / Agent.UNREACHABLE_NETWORKS_TABLE_FILENAME
    )

  @property
  def cell_known_networks(self) -> set[ipaddress.IPv4Network]:
    return _read_networks_file(
      self.log, self.experiment_uvn_dir / "log" / Agent.KNOWN_NETWORKS_TABLE_FILENAME
    )

  @property
  def cell_local_networks(self) -> set[ipaddress.IPv4Network]:
    return _read_networks_file(
      self.log, self.experiment_uvn_dir / "log" / Agent.LOCAL_NETWORKS_TABLE_FILENAME
    )
  ...
  def cell_fully_routed(self) -> bool:
    ...
    assert (
      len(unexpected_lans) == 0
    ), f"{len(unexpected_lans)} unexpected reachable network: {unexpected_lans}"
    return expected_lans == self.cell_reachable_networks

  def particle_file(self, cell: Cell, ext: str = ".conf") -> tuple[Path, Path]:
    artifacts_dir, artifacts_dir_c = self.test_file(
      f"{self.particle.uvn.name}__{self.particle.name}"
    )
    ...
    exec_command(
      [
        ...
        *(["-ti"] if interactive else []),
        ...
        "UNO_TEST_RUNNER=y",
        "-v",
        f"{self.experiment.root}:{self.experiment.RunnerRoot}",
        "-v",
        f"{self.experiment.test_dir}:{self.experiment.RunnerTestDir}",
        *(
          ["-v", f"{self.experiment_uvn_dir}:{self.experiment.RunnerRegistryRoot}"]
          if self.role in (HostRole.REGISTRY, HostRole.CELL)
          else []
        ),
  ...
  def stop(self) -> subprocess.Popen:
    ...
    return subprocess.Popen(
      [
        ...
        "SIGINT",
        "-t",
        str(self.experiment.config["container_stop_timeout"]),
        self.container_name,
      ]
    )
  ...
  def wait_ready(self) -> None:
    ...
    Timer(
      self.experiment.config["container_start_timeout"],
      0.5,
      lambda: self.ready,
      self.log,
      ...
      f"container {self.container_name} failed to activate",
    ).wait()
  ...
  def _init_host(self) -> None:
    self._init_common()
    self.install_default_route(dev=self.default_nic, route=self.default_network.router_address)

  def _init_router(self) -> None:
    # Kernel forwarding is expected to be already enabled on the host, because Docker needs it.
    assert (
      int(
        exec_command(["cat", "/proc/sys/net/ipv4/ip_forward"], capture_output=True)
        .stdout.decode()
        .strip()
      )
      == 1
    ...
    exec_command(
      ["iptables", "-A", "FORWARD", "-i", self.default_nic, "-o", self.default_nic, "-j", "ACCEPT"]
    )
    ...
    if self.upstream_network:
      ...
      self.install_default_route(dev=dev, route=self.upstream_network.router_address)
    else:
      exec_command(["ip", "route", "delete", "default"])
    ...
    def _enable_fwd(nic_in: str, nic_out: str):
      ...
      exec_command(["iptables", "-A", "FORWARD", "-i", nic_in, "-o", nic_out, "-j", "ACCEPT"])
    ...
    if self.default_network.designated_agent:
      ...
      for other_net in (
        *other_cell_networks,
        ...
        exec_command(["ip", "route", "add", str(other_net), "via", str(agent_addr)])
  ...
  def _init_particle(self) -> None:
    ...
    exec_command(["unzip", "-o", self.particle_package], cwd=self.test_dir)
  ...
  def _docker_exec_cmd(
    self, *args, user: str | None = None, interactive: bool = False, terminal: bool = False
  ) -> list[str | Path]:
    return [
      "docker",
      "exec",
  ...
  def popen(
    self, *args, user: str | None = None, capture_output: bool = False, **popen_args
  ) -> subprocess.Popen:
  ...
  def iperf_test(self, server: "Host", tcp: bool = True) -> None:
    with server.iperf_server() as iperf_server:
      self.log.activity("performing iperf3 {} test: {}", "TCP" if tcp else "UDP", server)
      test_len_exp = 5
      timer = Timer()
      timer.start()
      self.exec(
      ...
      test_len = timer.stop()
      self.log.info(
        "iperf3 {} test completed in {} seconds: {}",
        "TCP" if tcp else "UDP",
        test_len.total_seconds(),
        server,
      )
  ...
  @contextlib.contextmanager
  def uno_agent(
    self, wait_exit: bool = False, graceful: bool = True
    ...
    try:
      ...
      if wait_exit:
        self.uno_agent_wait_exit(
          timeout=self.experiment.config["container_stop_timeout"], graceful=graceful
        )
...
""",
  ...
  def uno_agent_request_stop(self) -> None:
    self.exec(
      "sh", "-exc", "[ ! -f /run/uno/uno-agent.pid ] || kill -s INT $(cat /run/uno/uno-agent.pid)"
    )
  ...
  def uno_agent_wait_exit(self, timeout: float = 60, graceful: bool = True) -> None:
    timer = Timer(
      timeout,
      1,
      lambda: not self.uno_agent_running(),
      self.log,
  ...
  def agent_httpd_test(self, agent_host: "Host") -> None:
    def _test_url(address: ipaddress.IPv4Address):
      output, output_c = self.test_file(
        f"{agent_host.container_name}-{address}-index.html", create=True
      )
    ...
    self.log.activity(
      "performing HTTPD test on {} addresses for {}: {}",
      len(agent_host.cell_addresses),
      agent_host,
      agent_host.cell_addresses,
    )
