...
def test_integration_basic_ping(experiment: Experiment, hosts: list[Host], uno_up):
  # Try to ping every host from every other host
  experiment.log.activity("testing PING communication between {} hosts: {}", len(hosts), [h.container_name for h in hosts])
  for host in hosts:
    for other_host in _other_hosts(host, hosts):
      host.ping_test(other_host)
  # Try also to ping every host from every agent
  agents = _agents(experiment)
...
def test_integration_basic_iperf(experiment: Experiment, hosts: list[Host], uno_up):
...
def test_integration_basic_ssh(experiment: Experiment, hosts: list[Host], uno_up):
